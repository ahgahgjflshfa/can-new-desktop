...
fn string_value(
    ...
    value(object, names).and_then(|item| {
        item.as_str()
            .map(str::to_owned)
...
pub async fn charge_complete_task(
    ...
    let response = client
        ...
        .bearer_auth(token)
        .json(&ChargeCompleteTaskRequestBody {
            status: "done",
            resolution_type: 1,
        })
        .send()
...
mod tests {
    ...
    fn completion_payload_is_charge_contract() {
        assert_eq!(
            serde_json::to_value(ChargeCompleteTaskRequestBody {
                status: "done",
                resolution_type: 1
            })
            .unwrap(),
            json!({"status":"done","resolutionType":1})
        );
    }
    ...
    fn serializes_structured_forbidden_error() {
        assert_eq!(
            serde_json::to_value(ChargeHttpError {
                status: 403,
