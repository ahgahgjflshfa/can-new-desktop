...
async function updateTask(serialNumber: number) { if (isActionPending.value) return; isActionPending.value = true; actionError.value = null; try { await notificationStore.completeChargeTask(serialNumber) } catch (error) { actionError.value = error instanceof Error ? error.message : (typeof error === 'object' && error && 'message' in error ? String(error.message) : String(error)) } finally { isActionPending.value = false } }
</script>
  ...
  <div class="min-h-full">
    ...
    <div class="mx-auto max-w-5xl px-6 py-6 md:px-8 md:py-8">
      ...
      <div v-else class="overflow-hidden rounded-[1.5rem] border border-[var(--app-border)] bg-[var(--app-surface)]">
        <div class="flex items-center justify-between border-b border-[var(--app-border)] p-4"><h2 class="flex items-center gap-2 text-xl font-semibold"><span class="i-mdi-lightning-bolt text-[var(--app-primary-strong)]" />{{ activeTab === 'active' ? '進行中' : '已完成' }}</h2><button type="button" class="rounded-lg px-3 py-1.5 text-[var(--app-muted)] hover:bg-[var(--app-hover)]" @click="refresh"><span class="i-mdi-refresh" /></button></div>
