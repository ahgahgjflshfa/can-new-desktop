...
describe('canTaskService', () => {
  ...
  test('uses the explicit CAN token for direct task fetch and completion', async () => {
    await fetchCanTasks('can-token')
    await completeCanTask('can-token', 42, true, 1)

    expect(invoke).toHaveBeenNthCalledWith(1, 'can_fetch_tasks', { token: 'can-token' })
  ...
  test('blocks IPC when the CAN token is missing', async () => {
    await expect(fetchCanTasks('')).rejects.toThrow('Q 潔淨立馬清')
